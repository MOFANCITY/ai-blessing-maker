...
- 微信开发者工具
- 微信小程序账号
- 后端 API 服务（可使用本项目的 Next.js 版本部署在 Vercel 上）
...
2. 注册 [微信公众平台](https://mp.weixin.qq.com/) 账号并创建小程序
3. 获取小程序 AppID

#### 2. 配置项目

1. 使用微信开发者工具打开项目中的 `wechat-miniprogram` 目录
2. 修改 `project.config.json` 文件中的 `appid` 字段为你的小程序 AppID
3. 修改 `utils/api-client.js` 文件中的 API 地址为你部署的后端服务地址

#### 3. 配置服务器域名

在微信公众平台的小程序后台，配置以下服务器域名：

- `request合法域名`：填写你的后端 API 服务域名
- `socket合法域名`：如使用 WebSocket，填写相应域名
- `uploadFile合法域名`：如使用文件上传功能，填写相应域名
- `downloadFile合法域名`：如使用文件下载功能，填写相应域名
...
1. 登录微信公众平台
2. 进入小程序管理后台
3. 在"开发管理"->"开发版本"中找到刚刚上传的版本
4. 点击"提交审核"
5. 填写审核信息并提交

#### 6. 发布上线

审核通过后，在"审核版本"中点击"发布"即可上线。

### 注意事项

1. 小程序包大小限制为 8MB，请注意优化资源文件
2. 网络请求必须使用 HTTPS 协议
3. 需要后端 API 支持 CORS 或部署在同一域名下
4. 可以使用本项目的 Next.js 版本作为后端服务，部署在 Vercel 上
