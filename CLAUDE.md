...
4. **安全防护**: 基于 Vercel KV 的速率限制 + 输入验证 + 安全响应头
...
**速率限制** (`middleware.ts`):
- 基于 Vercel KV 的分布式限流: 每分钟8次，每天50次
- IP级别精准控制: 基于真实IP进行限流统计
- 优雅降级: 即使 KV 服务不可用也能正常提供服务
- 自动过期清理: 防止数据堆积

**输入验证** (`lib/validation.ts`):
- 长度限制: 5-300字符，防止过长输入
- 提示词注入防护: 过滤常见的注入模式
- XSS防护: 过滤脚本标签和危险内容
- 文本标准化: 清理控制字符

**基础Web安全** (`next.config.js`):
- MIME类型嗅探防护
- 点击劫持防护
- XSS防护响应头

**高可用设计**:
- API故障转移: 主力API失败自动切换备用
- 错误处理: 不泄露内部信息
- 环境变量保护: API密钥安全存储
...
**Vercel部署注意事项**:
- 需要配置 Vercel KV 用于速率限制（免费套餐足够）
- 所有环境变量必须在 Vercel 项目设置中配置
- Edge Runtime 不支持 Node.js 特定 API
- 自动获得全球CDN加速和HTTPS
