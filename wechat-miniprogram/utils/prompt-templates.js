 ...
 * 创建智能模式的提示词
 * 基于用户的自由描述生成个性化的祝福语
 * @param {Object} options - 包含用户描述的请求参数
 * @returns {string} 完整的 AI 提示词文本
 */
function createSmartPrompt(options) {
 ...
 * @param {Object} options - 包含场景、目标人群和风格的请求参数
 * @returns {string} 完整的 AI 提示词文本
 */
function createTemplatePrompt(options) {
 ...
 * @param {Object} options - 完整的祝福语请求参数
 * @returns {string} 根据模式生成的对应提示词
 */
function createBlessingPrompt(options) {
  // 如果启用智能模式且有自定义描述，使用智能提示词
  if (options.useSmartMode && options.customDescription && options.customDescription.trim()) {
